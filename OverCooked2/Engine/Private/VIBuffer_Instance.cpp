...
void CVIBuffer_Instance::UpRise_Steam(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE tSubResource{};
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		pVertices[i].vRight.x += fTimeDelta * 1.5f;
		pVertices[i].vUp.y += fTimeDelta * 1.5f;

		if (pVertices[i].vRight.x >= 0.75f)
		{
			pVertices->isLived = false;
		}
...
void CVIBuffer_Instance::Dwarf(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE tSubResource{};
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		pVertices[i].vRight.x -= fTimeDelta * 2.5f;
		pVertices[i].vUp.y -= fTimeDelta * 2.5f;
...
void CVIBuffer_Instance::Be_Huge(_float fTimeDelta)
{
	D3D11_MAPPED_SUBRESOURCE tSubResource{};
