...
HRESULT CRenderer::Render()
{
	...
	if (FAILED(Render_UI_Model()))
		return E_FAIL;

#ifdef _DEBUG
	if (m_pGameInstance->Get_DIKeyState(DIK_Q) == EKeyState::PRESSING)
	{
...
HRESULT CRenderer::AfterRender()
{
	if (FAILED(Render_After_UI()))
		return E_FAIL;
...
HRESULT CRenderer::Render_UI()
{
	m_RenderObjects[RENDER_UI].sort([](CGameObject* pSour, CGameObject* pDest)->_bool {
			return ((CUI*)pSour)->Get_RendOrder() > ((CUI*)pDest)->Get_RendOrder();
		});
...
HRESULT CRenderer::Render_After_UI()
{
	m_RenderObjects[RENDER_AFTER_UI].sort([](CGameObject* pSour, CGameObject* pDest)->_bool {
		return ((CUI*)pSour)->Get_RendOrder() > ((CUI*)pDest)->Get_RendOrder();
		});

	for (auto& pRenderObject : m_RenderObjects[RENDER_AFTER_UI])
	{
		if (nullptr != pRenderObject)
			pRenderObject->Render();
		Safe_Release(pRenderObject);
	}
	m_RenderObjects[RENDER_AFTER_UI].clear();

	return S_OK;
}

HRESULT CRenderer::Render_UI_Model()
{
	for (auto& pRenderObject : m_RenderObjects[RENDER_MODEL_UI])
	{
		if (nullptr != pRenderObject)
			pRenderObject->Render();
		Safe_Release(pRenderObject);
	...
	m_RenderObjects[RENDER_MODEL_UI].clear();
