...
HRESULT CPart_Weapon::Initialize(void* pArg)
{
    ...
    if (FAILED(Add_Components(pWeaponDesc->strModelTag)))
        return E_FAIL;

    //m_pTransformCom->Set_Scaled(10.f, 10.f, 10.f);
    m_pTransformCom->Rotation(XMVectorSet(1.f, 0.f, 0.f, 0.f), XMConvertToRadians(180.f));
    m_pTransformCom->Set_State(CTransform::STATE_POSITION, _float4(5.f, 0.f, 2.5f, 1.f));

    return S_OK;
}

int CPart_Weapon::Tick(_float fTimeDelta)
{
    return EVENT_NONE;
}

void CPart_Weapon::Late_Tick(_float fTimeDelta)
{
...
HRESULT CPart_Weapon::Add_Components(wstring strModelTag)
{
    /* For.Com_Shader */
...
CPart_Weapon* CPart_Weapon::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CPart_Weapon* pInstance = new CPart_Weapon(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed To Created : CPart_Weapon"));
...
CGameObject* CPart_Weapon::Clone(void* pArg)
{
    CPart_Weapon* pInstance = new CPart_Weapon(*this);
