...
uint        g_iNumCol, g_iNumRow, g_iIndex;
...
PS_OUT PS_SLICE(PS_IN In)
{
    ...
    float fColSize = 1 / (float) g_iNumCol;
    float fRowSize = 1 / (float) g_iNumRow;
    
    In.vTexcoord.x = (g_iIndex % g_iNumCol) * fColSize + (In.vTexcoord.x * fColSize);
    In.vTexcoord.y = (g_iIndex / g_iNumCol) * fRowSize + (In.vTexcoord.y * fRowSize);
